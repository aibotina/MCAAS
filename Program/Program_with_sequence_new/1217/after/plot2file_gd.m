...
distCol = getUpTriangle(CC);
distHammingCol = getUpTriangle(CC);
rMatrix = size(CC,1);
...
% set(gca, 'Ytick', -0.1:0.15);
 ...
 for i = 1: nCluster
            index = virusCluster{i};
             scatter(mdsCorr(CIC(index),1), mdsCorr(CIC(index),2), colorVec(i), shapeVec(i));    
            text(mean(mdsCorr(CIC(index),1)),mean(mdsCorr(CIC(index),2)),clusterVec(i));
 end
